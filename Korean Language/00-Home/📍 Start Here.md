# Korean Language Study

Welcome to your Korean language learning vault. Navigate using the links below or the file explorer.

## 🚀 Quick Start

- **New to this vault?** Start with [[Quick Reference]]
- **Browse lessons** → [[Lessons by Date]]
- **Look up grammar** → [[Grammar Index]]
- **Search vocabulary** → [[Vocabulary Index]]

---

## 📚 Study Sections

### [[10-Lessons/📌 Lessons by Date|Lessons]]
All lessons organized chronologically (Jun–Aug 2026). Each lesson covers grammar and vocabulary for that session.

### [[20-Grammar/📌 Grammar Index|Grammar]]
Grammar topics broken down by type:
- Particles (조사)
- Verb endings (어미)
- Numbers & time
- Special verbs

### [[30-Vocabulary/📌 Vocabulary Index|Vocabulary]]
Over 190 words organized by category (family, food, verbs, adjectives, positions, etc.)

### [[40-Review/Practice & Review|Review]]
Practice exercises, drills, and common mistakes to avoid.

---

## 📊 Progress

| Section | Status |
|---------|--------|
| Lessons covered | Jun 18 – Aug 30, 2026 (9 sessions) |
| Grammar topics | 14 major areas |
| Vocabulary | ~190+ words |

---

## ⚡ Study Tips

1. **Build from foundations**: Start with [[00-Home/Quick Reference|particle system]] before moving to verbs
2. **Practice regularly**: Use [[40-Review/Conjugation Drills|conjugation drills]] to internalize endings
3. **Link learning**: Click `[[links]]` to explore connections between topics
4. **Use search**: Cmd/Ctrl+Shift+F to search across all files

---

**Last updated**: 2026-08-30 | **Total notes**: 45+
